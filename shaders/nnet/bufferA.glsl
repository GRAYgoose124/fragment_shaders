...
#iUniform float DECAY = 1.00 in { 0.00, 1.00 } 
...
#define RG OP(r, g, b)
#define GB OP(g, b, r)
#define P1 (BR * NU) - (cos(RG) * sin(GB))
#define P2 (RG * NU) - (cos(GB) * sin(BR))
#define P3 (GB * NU) - (cos(BR) * sin(RG))
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    // Field Pass - Physics
    vec4 lap = laplacian(P, iChannel0, R);
    col += lap * RHO;
    ...
    if (sign(iMouse.z) == 1. || iFrame == 0) {
        ...
        col = vec4(0.);
        #else 
        col += SOURCE(UV, M, 0.005, vec4(1., abs(cos(iTime)), abs(sin(iTime)), 0.));
        #endif
